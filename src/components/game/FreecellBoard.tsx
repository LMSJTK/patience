...
import { useFreecellStore, FreecellLocation } from '../../store/useFreecellStore';
...
function DraggableCard({ 
  ...
  const { attributes, listeners, setNodeRef, isDragging } = useDraggable({
    ...
    disabled: !card.isFaceUp || !isValidSequence(cardsToDrag),
  });
...
export default function FreecellBoard() {
  const { 
    ...
    initGame, autoMoveCard, isWon, handleDrop, undo, history
  } = useFreecellStore();

  const [activeDragData, setActiveDragData] = useState<{ location: FreecellLocation; cards: Card[] } | null>(null);
  ...
  const onDragStart = (event: DragStartEvent) => {
    const { active } = event;
    setActiveDragData(active.data.current as { location: FreecellLocation; cards: Card[] });
  };
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.data.current) {
      const from = active.data.current.location as FreecellLocation;
      const to = over.data.current as { type: 'tableau' | 'foundation' | 'freecell', index: number };
      handleDrop(from, to);
    }
  };
          ...
          <div className="flex gap-1 sm:gap-2 md:gap-4">
            {freeCells.map((card, i) => (
              <DroppableArea 
                ...
                onClick={() => {
                  if (card) {
                    autoMoveCard({ type: 'freecell', index: i });
                  }
                }}
              >
                {card && (
